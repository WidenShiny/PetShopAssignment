...
namespace SalesWinApp
{
    public partial class frmProducts : Form
    {
        ...
        {
            var comfirmationResult = MessageBox.Show("Are you sure you want to delete this product?", "Confirm Delete", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            var check = _orderDetailRepository.GetOrderDetails().FirstOrDefault(c => c.ProductId == CurrentGrid.ProductId);
            if (check == null && comfirmationResult == DialogResult.Yes)
            {
        ...
        private void frmProducts_Load(object sender, EventArgs e)
        {
            if (!NeedRefresh)
            {
				...
				CurrentGrid.Species = dgvProducts.Rows[CurrentRow].Cells[3].Value.ToString();
				CurrentGrid.UnitPrice = decimal.Parse(dgvProducts.Rows[CurrentRow].Cells[4].Value.ToString());
				CurrentGrid.UnitsInStock = int.Parse(dgvProducts.Rows[CurrentRow].Cells[5].Value.ToString());
			}
            else
            {
                txtSearch.Text = "";
        ...
        private void LoadAllProducts()
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    CurrentGrid.Species = dgvProducts.Rows[0].Cells[3].Value.ToString();
                    CurrentGrid.UnitPrice = decimal.Parse(dgvProducts.Rows[0].Cells[4].Value.ToString());
					CurrentGrid.UnitsInStock = int.Parse(dgvProducts.Rows[0].Cells[5].Value.ToString());
				}
            }
            catch (Exception ex)
            {
        ...
        private void dgvProducts_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex < _productRepository.GetProducts().Count && e.RowIndex >= 0)
            {
				...
				CurrentGrid.Species = dgvProducts.Rows[e.RowIndex].Cells[3].Value.ToString();
				CurrentGrid.UnitPrice = decimal.Parse(dgvProducts.Rows[e.RowIndex].Cells[4].Value.ToString());
                CurrentGrid.UnitsInStock = int.Parse(dgvProducts.Rows[e.RowIndex].Cells[5].Value.ToString());
        ...
        {
            this.dgvProducts.Columns[6].Visible = false;
        }
    }
}