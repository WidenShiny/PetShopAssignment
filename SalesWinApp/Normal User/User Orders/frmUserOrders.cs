...
namespace SalesWinApp.Normal_User
{
    ...
    {
        public IUserRepository _memberRepository { get; set; }
        ...
        private void frmUserOrders_Load(object sender, EventArgs e)
        {
            ...
            if (!isNull)
            {
                ...
                CurrentGrid.UserId = int.Parse(dgvOrders.Rows[CurrentRow].Cells[1].Value.ToString());
                ...
                CurrentGrid.TotalMoney = decimal.Parse(dgvOrders.Rows[CurrentRow].Cells[5].Value.ToString());
            }
            else
            {
        ...
        {
            var member = _memberRepository.Getusers().FirstOrDefault(c => c.Email == tmpEmail);
            var allOrders = _orderRepository.GetOrders().Where(c => c.UserId == member.UserId);
            try
            {
                ...
                else
                {
                    ...
                    CurrentGrid.UserId = int.Parse(dgvOrders.Rows[0].Cells[1].Value.ToString());
                    ...
                    CurrentGrid.TotalMoney = decimal.Parse(dgvOrders.Rows[0].Cells[5].Value.ToString());
                }
            }
            catch (Exception ex)
            {
        ...
        private void dgvOrders_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex < _orderRepository.GetOrders().Count && e.RowIndex >= 0)
            {
                ...
                CurrentGrid.UserId = int.Parse(dgvOrders.Rows[e.RowIndex].Cells[1].Value.ToString());
